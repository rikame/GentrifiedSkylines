...
namespace AdaptiveGentrification.Detours
{
    ...
    {
        [RedirectMethod]
        public override virtual void CalculateUnspawnPosition(ushort buildingID, ref Building data, ref Randomizer randomizer, CitizenInfo info, ushort ignoreInstance, out Vector3 position, out Vector3 target, out Vector2 direction, out CitizenInstance.Flags specialFlags)
        {
            ...
            if (flag1 && (!flag2 || randomizer.Int32(4U) == 0))
            {
                ...
                if (new Randomizer((int)buildingID << 6 | prop.m_index).Int32(100U) < prop.m_probability && data.Length >= prop.m_requiredLength)
                {
                    ...
                    if (!this.IsSomeBodyThere(position, ignoreInstance))
                    {
                        ...
                        Debug.Log("SF: " + specialFlags);
                        Debug.Log("SP: " + specialPlace.m_specialFlags);
